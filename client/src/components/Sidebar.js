...
const Sidebar = ({ isSidebarOpen, toggleSidebar, onChannelSelect }) => {
    ...
    const channels = user ? authChannels : guestChannels; 
    const setChannels = user ? setAuthChannels : setGuestChannels; 
    const setError = user ? setAuthError : setGuestError; 
    const error = user ? authError : guestError; 
    ...
    return (
        <div className={`sidebar ${isSidebarOpen ? 'open' : 'closed'}`}>
            ...
            {isSidebarOpen && ( // Render content only if sidebar is open
                <>
